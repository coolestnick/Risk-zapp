...
contract ShardeumMarketplace {
    IShardeumRegistry public registry;
    ...
    modifier onlyDomainOwner(string memory name) {
        (address domainOwner,,,, ) = registry.getDomain(name);
        require(domainOwner == msg.sender, "Not domain owner");
        _;
    }

    constructor(address _registry) {
        registry = IShardeumRegistry(_registry);
        owner = msg.sender;
    }
    ...
    // End auction and transfer domain
    function endAuction(string memory name) external {
        ...
        if (auction.highestBidder != address(0)) {
            ...
            // Transfer domain through registry
            registry.transfer(name, auction.highestBidder);
    ...
    // Accept an offer
    function acceptOffer(string memory name, uint256 offerIndex) external onlyDomainOwner(name) {
        ...
        // Transfer domain
        registry.transfer(name, offer.buyer);
