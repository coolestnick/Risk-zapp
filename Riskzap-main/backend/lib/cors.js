const defaultOrigins = 'https://risk-zapp.vercel.app,http://localhost:8081,http://localhost:8082,http://localhost:5173,http://localhost:3000';

// List of allowed origins for verification
const allowedOrigins = [
  'https://risk-zapp.vercel.app',      // Production frontend
  'https://risk-zapp-lzjc.vercel.app', // Backend URL (for testing)
  'https://risk-zapp-fn3s.vercel.app', // Current backend deployment
  'http://localhost:8081',             // Local development
  'http://localhost:8082',             // Local development
  'http://localhost:5173',             // Vite dev server
  'http://localhost:3000'              // Alternative dev port
];

function setCorsHeaders(res, origin) {
  // Check if origin is allowed, otherwise allow all with *
  const isAllowedOrigin = origin && allowedOrigins.some(allowed =>
    origin.startsWith(allowed) || allowed === origin
  );

  if (isAllowedOrigin) {
    res.setHeader('Access-Control-Allow-Origin', origin);
    res.setHeader('Access-Control-Allow-Credentials', 'true');
  } else {
    // For non-credentialed requests, use wildcard
    res.setHeader('Access-Control-Allow-Origin', '*');
  }

  res.setHeader('Access-Control-Allow-Methods', 'GET, POST, PUT, PATCH, DELETE, OPTIONS');
  res.setHeader('Access-Control-Allow-Headers', 'Content-Type, Authorization, X-Requested-With, Accept');
  res.setHeader('Access-Control-Max-Age', '86400');

  return res;
}

function handleCors(req, res) {
  const origin = req.headers.origin;
  setCorsHeaders(res, origin);
  
  // Handle preflight requests
  if (req.method === 'OPTIONS') {
    res.status(200).end();
    return true; // Indicates this was a preflight request
  }
  
  return false; // Not a preflight request
}

module.exports = {
  setCorsHeaders,
  handleCors,
  allowedOrigins
};