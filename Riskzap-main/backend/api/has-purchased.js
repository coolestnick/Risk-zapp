...
module.exports = async function handler(req, res) {
  ...
  return await withMonitoring(async () => {
    ...
    const [user, policyCount] = await withDbMonitoring(async () => {
      const userQuery = db.collection('users').findOne({ walletAddress: normalizedAddress });
      const policyQuery = db.collection('policies').countDocuments({ walletAddress: normalizedAddress });
