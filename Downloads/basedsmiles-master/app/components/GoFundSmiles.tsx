...
const RECIPIENT_ADDRESS = "0x766FCCd9BEA80F35A4aE34913aDd8ee825ECbEeA";
...
const GoFundSmiles = ({ wallet }: GoFundSmilesProps) => {
  ...
  useEffect(() => {
    const fetchBalances = async () => {
      ...
      try {
        ...
        const contract = new ethers.Contract(RECIPIENT_ADDRESS, CONTRACT_ABI, provider);

        // Fetch contract balance (total funds)
        const totalFundsRaw = await contract.getContractBalance();
        const formattedTotalFunds = ethers.utils.formatEther(totalFundsRaw);
        setTotalFunds(parseFloat(formattedTotalFunds).toFixed(4));

        // Fetch user SHM balance
        const signer = provider.getSigner();
        const userAddress = await signer.getAddress();
        const userBalanceRaw = await provider.getBalance(userAddress);
        const formattedUserBalance = ethers.utils.formatEther(userBalanceRaw);
        setUserBalance(parseFloat(formattedUserBalance).toFixed(4));
      } catch (error) {
  ...
  const handleDonate = async () => {
    ...
    try {
      const provider = await wallet.getEthersProvider();
      const signer = provider.getSigner();
      const contract = new ethers.Contract(RECIPIENT_ADDRESS, CONTRACT_ABI, signer);

      const parsedAmount = ethers.utils.parseEther(amount);

      // Call fundContract function with SHM value
        ...
        <div className="bg-gradient-to-r from-pink-100 via-[#FFE5E5] to-pink-100 border-[3px] border-black rounded-lg p-8 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] relative overflow-hidden">
          ...
          <div className="bg-white/40 p-6 rounded-lg border-2 border-black max-w-md mx-auto">
            <div className="mb-2 flex justify-between items-center">
              <span className="text-sm font-medium text-gray-600">Your SHM Balance:</span>
              <span className="font-bold">
                {userBalance} SHM
              </span>
            </div>
            ...
            <div className="flex gap-4 items-center">
              <div className="relative flex-1">
                <Input
                  ...
                  placeholder="Amount to donate (SHM)"
                  className="pr-20 border-2 border-black text-lg font-medium h-12"
                  min="0"
                  step="0.1"
                />
                <div className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center gap-2">
                  ...
                  <span className="text-sm font-bold">SHM</span>
                </div>
              </div>
              <Button
